...
from source.base_karnaugh_map_ceil_builder import BaseKarnaughMapBuilder
...
class KarnaughMapBuilder(BaseKarnaughMapBuilder):
    def build(self, vector: str) -> KarnaughMap:
        ...
        result = KarnaughMap()
        result._column_vars = [f"x{i+1}" for i in range((var_count + 1) // 2)]
        result._row_vars = [f"x{i+1}" for i in range((var_count + 1) // 2, var_count)]
        row = []
